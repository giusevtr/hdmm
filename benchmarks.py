...
def adult_benchmark():
    ...
    data = Dataset.load('adult').project(cols)
    domain = data.domain
...
def adult2():
    data = Dataset.load('adult').drop('education')
...
def adult_hdmm():
    data = Dataset.load('adult').drop(['education-num'])
